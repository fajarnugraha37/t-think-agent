...
def write_opencode():
    ...
    fm={'description':'Govern an evidence-gated multi-agent software change with adaptive lanes and composite review phases.','mode':'primary','temperature':0.1,'permission':{'read':'allow','glob':'allow','grep':'allow','list':'allow','skill':'allow','edit':'ask','bash':'ask','external_directory':'deny','task':task}}
    ...
    for name,a in AGENTS.items():
        mode=a['permissions']['source_write']['mode']; perm={'read':'allow','glob':'allow','grep':'allow','list':'allow','skill':'allow','external_directory':'deny','task':'deny','bash':'ask'}
        perm['edit']='ask' if mode=='approved_targets_only' else 'deny'
