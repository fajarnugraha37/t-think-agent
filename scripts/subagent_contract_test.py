...
def packet(phase,agent,skill,mode='deny',approved=None,generated=None,lane='full',track=None,next_state=None):
 return {'schema_version':'2.0.0','identity':{'work_id':'W-1','run_id':'RUN-1','invocation_id':'INV-1','parent_agent':'t-think','target_agent':agent},'lifecycle':{'current_phase':phase,'skill':skill,'attempt':1,'lane':lane,'artifact_mode':{'quick':'compact','standard':'normal','full':'exhaustive'}[lane],'expected_next_state':next_state or phase,'track':track},'objective':{'statement':'Complete the authorized bounded phase contract','completion_criteria':['Produce schema-valid evidence-backed output']},'workspace':{'root':'.','active_work_directory':'.t-think/W-1','discovery':{'respect_vcs_ignore':False,'include_untracked':True,'include_ignored':True}},'permissions':{'workspace_read':'allow','outside_workspace':'deny','source_write':mode,'governance_artifact_write':'.t-think/W-1/**','approved_write_targets':approved or [],'generated_output_paths':generated or [],'protected_paths':['.git/**'],'ignored_file_access':{'mode':'allow','human_approval_ref':None},'spawn_subagent':'deny'},'artifact_inputs':[],'output_contract':{'schema':f'skills/{skill}/schemas/output.schema.json','artifact_path':f'.t-think/W-1/artifacts/{track or phase.lower()}.yaml','result_path':'.t-think/W-1/results/INV-1.yaml','boundary_report_path':'.t-think/W-1/boundary-reports/INV-1.yaml'},'stop_conditions':['missing evidence','new semantic decision']}
def activity(**kw):
...
def main():
 ...
 bad_scope=packet('INVESTIGATION','t-investigator','t-investigation',next_state='SYSTEM_MODEL');bad_scope['permissions']['governance_artifact_write']='.t-think/**';check(bool(validate_delegation(bad_scope)),'broad .t-think governance scope accepted',issues)
 root_activity=activity(governance_artifact_changes=['.t-think/investigation.yaml']);check(audit_boundaries(inv,root_activity)['status']=='FAIL','root-level governance artifact accepted',issues)
 passed.append('active work-directory isolation')
